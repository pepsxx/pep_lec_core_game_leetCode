package Pp_T04_LeeCod.Pp_0070.Pp_v1;
...
class Solution {
    public int climbStairs(int n) {
        ...
        while (chkZer < n-2) {
            String strBin = Integer.toBinaryString(itr++);
            sum = 0;
            chkZer = 0;
            for (int b = 0; b < strBin.length(); b++) {
                if (strBin.charAt(b) == '1') {
                    sum += 2;
                } else if (strBin.charAt(b) == '0') {
                    sum++;
                    chkZer++;
                }
                if ((sum - (strBin.length()-1-b)) > n) break;
            }
            if (sum <= n) { cntVar++;
//                System.out.println(strBin.length() + " " + strBin + " cntVar++ = " + cntVar);
            }
        }
        return cntVar;
    }
